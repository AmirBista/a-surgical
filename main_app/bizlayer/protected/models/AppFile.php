 ...
 */
class AppFile extends YIGBaseModel
{
	/**
	...
	public function getListData($params) {
    	...
    	if ($for_count == true){
			...
			$sql .= $this->getSyncStatusColumn('t19', $synctime, $user_id);
    	}

    	$sql .= " FROM app_file AS t19";

		/*if(!empty($synctime)) {
        	$sql .= " WHERE (t3.updated_datetime > '{$synctime}'";
        }else {*/
			$sql .= " WHERE  (t19.delete_flg = 0 ";
       // }

 		$sql .= " AND t19.ref_record_id = '{$ref_record_id}'";
		if (!empty($sql_where))
		{
			$sql .= " AND {$sql_where}";
		}
    	...
    	if ($for_count != true){
			$default_sort_order = "t19.display_order ASC";
			$sort_order = $this->getSortOrder('t19', $sort, $default_sort_order);
			if (!empty($sort_order)){
				$sql .= $sort_order;
			}
