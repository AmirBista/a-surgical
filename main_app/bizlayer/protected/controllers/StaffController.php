<?php
class StaffController extends PrivateController {
    public function __construct($arg = NULL) {
        parent::__construct($arg);
        $this->_datagrid_id = 9;
        $this->_keyword_searchable_fields = array();/*array("column_5_01","column_5_02","column_5_03")*/;
        // $this->_unique_columns = array("column_5_02");
    }
    public function actionGetRecord() {
        $code = Yii::app()->request->getParam('code',0);
        $result = array('success' => FALSE,'msg' => t('general','Invalid Company Code'));
        $record = Staff::model()->findByAttributes(array('column_5_02' => $code,'delete_flg'=> 0));
        if(!empty($record)){
            $result = array('success' => TRUE,'data' => $record);
        }
    ...
    */
    ...
    private function _getList($return_array=array()) {
        ...
        $sql_where = $clientsModel->getWhereCondition($getWhereConditionParams);
        $col_info = array();
        ...
        $listDataParams = array(
            ...
            'billClosingDay'        => null,
            'forInvoiceBill'        => null,
            'forInvoicePayment'     => null
        );
        
        $total = $clientsModel->getListData($listDataParams);
        ...
        //end pagination
        $recordData = $clientsModel->getListData($listDataParams);
