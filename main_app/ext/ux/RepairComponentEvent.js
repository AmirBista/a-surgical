Ext.define('Ext.ux.RepairComponentEvent', {
    // extend:'YBase.controller.OrderMasterEntryController',
    alternateClassName: 'Ext.RepairComponentEvent',
    // singleton: true,
    absCmp: null,
    controller: null,
    // @private
    ...
    onComboFormFieldSelect: function(fld,record){
        ...
        else if(itemId == "column_12_13"){
            me.mapCustomerRecords(absCmp,record,false);
        }
        else if (itemId == "column_12_04"){
            me.mapOrderCodeRecords(absCmp,record,false);
        }
        else if (itemId == "column_12_31"){
            me.mapItemCodeRecords(absCmp,record,false);
        }
    },
    ...
    onFormFieldBlur: function(fld,e){
        ...
        // DISCOUNT
        if(itemId=='column_1_34'){
            me.calculateTotal();
        }
        // Payment Amt
        else if(itemId=='column_1_40'){
            var fld_balance = absCmp.query('textfield[itemId=column_1_41]')[0],
                fld_total = absCmp.query('textfield[itemId=column_1_36]')[0],
                totalAmt = fld_total.getValue(),
                balance = parseInt(totalAmt || 0) - parseInt(fldVal || 0);
            fld_balance.setValue(balance);
        }else if(itemId == 'column_12_12'){
    ...
    */
    ...
    mapOrderCodeRecords: function(absCmp,record,is_code){
        ...
        if(!Ext.isEmpty(record))
            ...
            var mapperArr = YBase.utility.DataMapperHelper.repairOrderCodeDataMapper;
            YBase.utility.EntryPanelHelper.mapRecords(absCmp,mapperArr,data);
    },
    mapItemCodeRecords: function(absCmp,record,is_code){
        var me = this,
            params = {},
            data = null;
        if(!Ext.isEmpty(record))
                data = is_code == true ? record.data : record[0].raw;
            var mapperArr = YBase.utility.DataMapperHelper.repairItemDetailMapper;
            YBase.utility.EntryPanelHelper.mapRecords(absCmp,mapperArr,data);
    },
    ...
    mapCustomerRecords: function(absCmp,record,is_code){
        var me = this,
            mapperArr = YBase.utility.DataMapperHelper.repairCustomerDetailMapper;
            data = null;
        if(!Ext.isEmpty(record))
            data = is_code == true ? record.data : record[0].raw;
        YBase.utility.EntryPanelHelper.mapRecords(absCmp,mapperArr,data);
    ...
    hasDuplicateRecord: function(store,selectedRecord,record){
        var index = store.findBy(function(rec,id){
                    if(rec.id != this.selectedRec.id && rec.data.column_2_06 == this.record.data.column_5_05 && rec.data.column_2_27 == this.record.data.column_5_16 && rec.data.supplier_id == this.record.raw.supplier_id)
                        return true;
    ...
    onEntryDetailGridStoreUpdate : function( store, record, operation, modifiedFieldNames, eOpts){
        ...
        if(modifiedFldName == 'column_2_19' ||
         modifiedFldName == 'column_4_19'){
            /*Do nothing.*/
            return;
        }
        /*
        ...
        * @column_2_14::rate
        */
        else if(modifiedFldName == "column_2_14" || modifiedFldName == "column_2_15" || modifiedFldName == "column_2_17"){
    ...
    onGridCellBeforeEdit: function(editor,e){
        ...
        /*Product Name*/
            ...
            var editorxtype = e.record.data.column_2_07 != Ext.LANG.entryPanel.productType.type1 ? {xtype : 'combobox'} : {xtype: 'textfield'},
    ...
    init:function (controller) {
        ...
        var orderDetailCb = me.controller.absCmp.query('combobox[itemId=column_12_04]')[0],
            orderDtailCbStore = orderDetailCb.getStore(),
            extraParams = orderDtailCbStore.getProxy().extraParams  || {};
        extraParams['forRepairEntryPanel'] = 1;
