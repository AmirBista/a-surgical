...
Ext.define('Ext.ux.Router', {
    ...
    init: function(app) {
        ...
        me.on('beforedispatch', function(token, route, params) {
    ...
    parse: function(token) {
        var route, matches, params, names, j, param, value, rules,p,
            me      = this,
            routes  = me.routes,
            i       = 0,
            len     = routes.length;
            ...
            if (route.regex) {
                ...
                if (matches) {
                    ...
                    while (j < names.length) {
                        ...
                        var queryStrPair=value.split('&');
                        for(var counter=0;counter<queryStrPair.length;counter++){
                            p=queryStrPair[counter].split("=");
                            if(p[1]=="true" || p[1]=="false")
                                p[1] = p[1] == "true" ? true : false;
                            params[p[0]]=p[1];
                            /*if(p.length==2){
                                //Assign value from queryString
                                params[p[0]]=p[1];
                            }
                            else{
