...
Ext.define('Ext.ux.grid.GridHeaderFilters',{
   ...
   */
    ...
    */
    ...
    init: function(grid)
    {
        ...
        */
        ...
        this.grid.on({
            ...
            columnshow: this.adjustFilterWidth            
        //            beforestatesave: this.saveFilters,
        ...
        Ext.apply(this.grid,
        {
            ...
            setHeaderFilter: function(sName, sValue, applyFilter)
            {
                if(!this.headerFilterPlugin)
                    return;
                var fd = {};
                fd[sName] = sValue;
                this.headerFilterPlugin.setFilters(fd, applyFilter);
            },
            /**
    ...
    {
        var columns = this.grid.headerCt.getVisibleGridColumns();
        for(var c=0; c < columns.length; c++)
        {
            var column = columns[c];
    ...
    setFilters: function(filters, applyfilter)
    {
        if(!filters)
            return;
        if (Ext.isEmpty(applyfilter))
            applyfilter = true;
    ...
    parseFilters: function()
    {
        ...
        for(var fn in this.fields)
        {
            ...
            if(!field.isDisabled()){
                if((!Ext.isEmpty(field.dateFormat) && !Ext.isEmpty(field.rawValue)) ||
                    (field.xtype == 'timefield' && !Ext.isEmpty(field.rawValue))){
                // if(!Ext.isEmpty(field.dateFormat) && !Ext.isEmpty(field.value)){
                    /*formating the date*/
                    // filters[field.filterName] = Ext.Date.format(field.value,Ext.LANG.dateFormat.defaultDateFormat);
                    filters[field.filterName] = field.rawValue;
                    // filters[field.filterName] = field.value.toString(Ext.LANG.dateFormat.defaultDateFormat);
                }else{
                    var val = field.getValue(), filter_name = field.filterName;
                    //if column name not starts width column_ then add table alias also
                    if (field.filterName.indexOf('column_') !== 0 && !Ext.isEmpty(field.column.table_alais)){
                        filter_name = field.column.table_alais + filter_name;
                    }
                    if (typeof val !== 'number') {
                        ...
                        filters[filter_name] = field.getValue();
                    } else {
                        filters[filter_name] = field.getValue();
                    }
                }
